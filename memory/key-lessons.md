...
- **脚本**：`/Users/mac/.openclaw/workspace/skills/ima-note/scripts/write_diary.py`
...
- 脚本：`/Users/mac/.openclaw/auto-backup.sh`
...
## IMA知识库读文章的标准流程（2026-04-20 新增）
- 遇到"读知识库里的文章"→ 先用 IMA `search_knowledge` 找到文章
- IMA API 存的是**元数据**（标题、ID、摘要），不是全文
- 正确路径：IMA 找到原文 URL（微信公众号被腾讯新闻转发后的链接）→ 再用 `web_fetch` 读正文
- **不要**一上来就用浏览器/web_search，先想清楚工具能力边界
- 微信文章在 IMA 的 media_type=6（wechatarticle），存的是转发后的腾讯新闻链接

## 工具能力边界速查（2026-04-20 更新）
- IMA `search_knowledge` → 能找到文章**在哪**，不能读**正文**
- 腾讯新闻转发页（news.qq.com）→ `web_fetch` 可直接读正文
- 搜狗微信搜索 → 只能看到摘要，需点击跳转
- 浏览器 → 完整渲染 JS，但操作复杂；有登录态用 profile=openclaw

## 任务铁律强化（2026-04-20）
遇到内容获取任务 → 先想清楚"我要的东西什么工具能直接拿到"
→ 不确定工具能力边界时，不乱试，先问
